#ifndef _VPF_REFINEDREADER_H_
#define _VPF_REFINEDREADER_H_
...
struct VPFRefinedReader
{
	static void ReadFile(std::ifstream* fin, std::vector< ImmersedSphere<double> >& spheres);
};

void VPFRefinedReader::ReadFile(std::ifstream* fin, std::vector< ImmersedSphere<double> >& spheres)
{
	...
	while(std::getline(*fin, row))
	{
		...
		for(int column = 1; column != 12; ++column)
		{
			...
			if(column == 1)
				StringUtility::ToTargetType<double>(word, sphereIndex);
		...
		spheres.push_back(ImmersedSphere<double>(sphereIndex,position, velocity, radius));
	}
